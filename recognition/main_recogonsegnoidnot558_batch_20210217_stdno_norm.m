...
%     datapath='.\input_withID\'; str_flagID='withID';
    ...
    for iter_file=1:length(L)
        ...
        filepath_apo_sub_gt="D:\Vaa3d\vaa3d_tools\released_plugins\v3d_plugins\C_elegent\c_elegent2\main_c_elegent\itera6\B0310.2_RW10695_20131123_0002\affine_marker_label.txt";
...
%     
   ...
   load("B0310.2_RW10695_20131123_0002_rpm_mse.mat");    
%    vx=vx;
    m=m;
    m_std=m;
    for i=1:ntarcell
...
%     ----------------------------------------------------------------------
    ...
    for iter=1:3
        ...
        X_sub=X_sub_bk(:,ind_tar2sub(ind_tar2sub_valid));%reorder according to current matching result
         X_sub_index=X_sub_bk(:,ind_tar2sub(ind_tar2sub_valid));%reorder according to current matching result
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%            
    ...
    [w,d,k]  = update_tps( X_sub_index', X_sub',lamda1);
     vx = update_marker(X_sub_bk',X_sub_index',w,d);
      X_sub_bk_paffine=vx';  
...
%     hold off;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        arr_pos_var_pwaff=arr_pos_std_pwaff.^2; clear arr_pos_std_pwaff
         ...
         load('.\traindata\man2atlas_pwaffine_meanstd_norm_sc.mat','arr_pos_std_pwaff');
...
%         [Cost]=ShapeContext3D(X_sub_bk_paffine',X_tar');
%          
...
%         end
        ...
        for i=1:length(X_sub_gt1)
            plot3([X_sub_gt_or(1,i);X_sub_pre(1,ind_tar2sub(i))],...
                  [X_sub_gt_or(2,i);X_sub_pre(2,ind_tar2sub(i))],...
                  [X_sub_gt_or(3,i);X_sub_pre(3,ind_tar2sub(i))],'m');
        end
        xlabel('x'),ylabel('y'),zlabel('z')
        axis('equal'); grid on; set (gca, 'box', 'on');
        hold off;
