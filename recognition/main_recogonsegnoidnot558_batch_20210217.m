  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    clear X_tar_piece xmax_piece xmin_piece T X_sub_piece
        ...
        for j =1:length(X_sub_bk_paffine)
         ...
         for j =1:length(X_tar)
             x=length(X_tar(X_tar(1,:)<X_tar(1,j)));
            X_tar_pp(1,j)=x/(558-x);
            y=length(X_tar(X_tar(2,:)<X_tar(2,j)));
            X_tar_pp(2,j)=y/(558-y);
            z=length(X_tar(X_tar(3,:)<X_tar(3,j)));
    ...
    %             exponential_term=0.5*(xyzdiff_atlas2topre.^2./(1^2*repmat(sum(arr_pos_var_pwaff_sc(:,i)),1,nsubcell)));
