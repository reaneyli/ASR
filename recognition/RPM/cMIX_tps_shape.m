function [o1,o2,o3,o4] =  cMIX_tps_shape ( in1,in2,in3,in4,in5,in6,in7,in8,X_sub_gt,X_tar_shape_all,X_sub_shape_all)
...
vy = y;
vx_affinw=x;
vX_sub_shape_all=X_sub_shape_all;
...
while (flag_stop ~= 1)
    for i=1:perT_maxit     % repeat at each termperature.
        % Given vx, y, Update m:
        m = cMIX_calc_m (vx, y, T, m_outliers_row, m_outliers_col,T_SC,X_tar_shape_all,vX_sub_shape_all);
        ...
        for i_warp_shape=1:6
            befor_marker=X_sub_shape_all{i_warp_shape};
              [vx_affinw,after_marker] = cMIX_warp_pts (trans_type, befor_marker', z, c_tps, d_tps, w, sigma);
              vX_sub_shape_all{i_warp_shape}=after_marker';     
        end
...
function [m, m_outliers_row, m_outliers_col] = cMIX_calc_m ...
    (vx, y, T, m_outliers_row, m_outliers_col,T_local,X_tar_shape_all,X_sub_shape_all)
...
%     y_tmp=y_tmp+(vx(:,it_dim) - y(:,it_dim)').^2+1000*T* ((vx(:,it_dim) - y(:,it_dim)').^2)./(20^2*repmat((arr_pos_var_pwaff(it_dim,:)),xmax,1));%-(T_std*ones(xmax,1) * arr_pos_var_pwaff(:,it_dim)' ));./(20^2*repmat((arr_pos_var_pwaff_sc(:,i)),1,nsubcell))
...
% for shape_i=1:6
%     shape_tmp=X_sub_shape_all{shape_i};
%     shape_tmp=shape_tmp';
%     tar_shape_tmp=X_tar_shape_all{shape_i};
%      tar_shape_tmp=tar_shape_tmp';
%     for it_dim=1:dim
%         y_tmp=y_tmp+(shape_tmp(:,it_dim) - tar_shape_tmp(:,it_dim)').^2;%-(T_std*ones(xmax,1) * arr_pos_var_pwaff(:,it_dim)' ));
%     end
% end
% y_tmp=y_tmp./7;
