...
%             S=apo_sub{ind_atlas2validsub_pre(i)};
    ...
    for(rpm_iter=1:3)
            ...
            [c,d,vx,m]=cMIX_tps_shape (X_sub_iter,X_tar',frac,T_init,T_final,lamda1_init,lamda2_init,disp_flag,X_sub_gt',X_tar_shape_all,X_sub_shape_all);
                        save m
...
%                             +(shape_prob_sc-min(min(shape_prob_sc)))./(max(max(shape_prob_sc))-min(min(shape_prob_sc))));
            ...
            clear X_sub_intensity X_sub_or2tar X_sub_piece_all  X_tar_intensity  yy
            ...
            X_sub=X_sub_or(:,ind_tar2sub(ind_tar2sub_valid));%reorder according to current matching result
              ...
              lamda1=0.1;%20*0.5^iter;
                ...
                vx = update_marker(X_sub_or',X_sub',w,d);
                X_sub_iter=vx';
    end
