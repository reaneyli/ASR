...
%         hold off;
        %do Recog
        ind_atlas2validsub_pre=RecogOnPosNoID(X_tar, X_sub);%noGT
%         ind_atlas2validsub_pre=RecogOnPosNoID(X_tar, X_sub, ind_atlas2validsub_man);%hasGT
        
        %[noGT]print recog result
        %fill recong ID to apo_sub
        for i=1:length(apo_atlas)
            ind_apo_sub=ind_validcell(ind_atlas2validsub_pre(i));
            apo_sub{ind_apo_sub}.name=apo_atlas{i}.name;
        end
...
function ind_tar2sub=RecogOnPosNoID(X_tar, X_sub, ind_tar2sub_man)
    ...
    %whether provided with groundtruth (manual anno for accu cpt)
    b_hasGT=1;
    if nargin<3
        b_hasGT=0;
    end
